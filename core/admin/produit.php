...
if(isset($_POST['action']) && $_POST['action'] == 'add-produit')
{
    ...
    {
        $statut_global = 2;
    }elseif($date_sortie < $date_format->date_jour_strt() && $date_sortie >= $date_format->date_jour_strt()-2505600)
    {
        $statut_global = 4;
    }else{
        $statut_global = 1;
...
if(isset($_POST['action']) && $_POST['action'] == 'edit-produit')
{
    ...
    if($sql_produit == 1 AND $sql_caracteristique == 1 AND $sql_categorie == 1 AND $sql_subcategorie == 1)
    {
        ...
        header("Location: ../../index.php?view=admin_sha&sub=produits&data=edit_produit&ref_produit=$ref_produit&error=edit-produit&text=$text");
    }

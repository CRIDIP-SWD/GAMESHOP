...
if($response)
{
    if($response['CHECKOUTSTATUS'] === 'PaymentActionNotInitiated')
    {
        ...
        if($timestamp >= $timestamp30)
        {
            ...
            $paiement = $paypal->request('CreateRecurringPaymentsProfile', array(
                "TOKEN"                     => $_GET['token'],
                "PAYERID"                   => $_GET['PayerID'],

                "PROFILESTARTDATE"  => gmdate("Y-m-dTH:i:sZ", time()+86400),
                "PROFILEREFERENCE"  => $num_commande,
                "BILLINGPERIOD"     => "Month",
                "BILLINGFREQUENCY"  => "3",
                "AMT"               => $cmd[0]->total_commande + $cmd[0]->prix_envoie,

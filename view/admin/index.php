...
<?php if(isset($_GET['sub']) && $_GET['sub'] == 'client'): ?>
    ...
    <?php if(isset($_GET['data']) && $_GET['data'] == 'view_client'): ?>
        ...
        <section role="main" class="content-body">
            ...
            <section class="panel panel-featured panel-featured-primary">
                <header class="panel-heading">
                    <h2 class="panel-title">LISTE DES RESERVATIONS</h2>
                </header>
                <div class="panel-body">
                    <div class="well">
                        <button type="button" class="mb-xs mt-xs mr-xs modal-basic btn btn-primary" href="#add-commande"><i class="fa fa-plus-circle"></i> Nouvelle Commande</button>
                    ...
                    <div class="">
                        <table class="table table-bordered table-striped mb-none" id="datatable-default">
                            ...
                            <tbody>
                            <?php
                            $sql_resa = $DB->query("SELECT * FROM client_reservation WHERE idclient = :idclient", array(
                                "idclient"  => $idclient
                            ));
                            foreach($sql_resa as $resa):
                                if($resa_cls->count_resa($idclient) == 0){
                                ?>
                                <tr>
                                    <td colspan="4" style="font-size: 9px; font-style: italic; text-align: center;">Aucun article réservé ou en précommande</td>
                                </tr>
                                <?php }else{ ?>
                                <tr>
                                    <td><?= $cmd->num_commande; ?></td>
